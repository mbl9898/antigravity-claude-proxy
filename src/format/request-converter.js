...
export function convertAnthropicToGoogle(anthropicRequest) {
    ...
    if (isThinking) {
        if (isClaudeModel) {
            ...
            // If request has very low max_tokens (e.g. title generation <= 100) and no explicit thinking requested, disable thinking
            if (max_tokens && max_tokens <= 100 && !thinking?.type && !thinking?.budget_tokens) {
                ...
                const currentMaxTokens = googleRequest.generationConfig.maxOutputTokens;
                if (currentMaxTokens && currentMaxTokens <= thinkingBudget) {
                    googleRequest.generationConfig.maxOutputTokens = thinkingBudget + (max_tokens || 4096);
                }
            }
        }
    }
